...
func (ae *ArgvEngine) parseInternalLong(a string, argv []string, ac ArgvContainer) error {
	ha := OPTIONAL
	ch := -1
	var oa string
	...
	if i != -1 {
		if i+1 > len(a) {
			return errors.New("unexpected argument '--" + a + "'")
		}
		oa = a[i+1:]
		a = a[0:i]
	}
	...
	if ch == -1 {
		return errors.New("unregistered option '--" + a + "'")
	}
	if len(oa) > 0 && ha == NOARG {
		return errors.New("option '--" + a + "' unexpected parameter: " + oa)
	}
	if len(oa) == 0 && ha == REQUIRED {
		if ae.index+1 >= len(argv) {
			return errors.New("option '--" + a + "' missing parameter")
		}
		oa = argv[ae.index+1]
		ae.index++
	}
...
func (ae *ArgvEngine) parseInternalShort(a string, argv []string, ac ArgvContainer) error {
	ha := OPTIONAL
	ch := -1
	if a[0] == '=' {
		return errors.New("unexpected argument '-" + a + "'")
	...
	if len(oa) > 0 && ha == NOARG {
		return errors.New("option '-" + a[0:1] + "' unexpected parameter: " + oa)
	}
	if len(oa) == 0 && ha == REQUIRED {
		if ae.index+1 >= len(argv) {
			return errors.New("option '-" + a[0:1] + "' missing parameter")
		}
		oa = argv[ae.index+1]
		ae.index++
	}
...
func (ae *ArgvEngine) parseInternal(a string, argv []string, ac ArgvContainer) error {
	if len(a) == 1 {
